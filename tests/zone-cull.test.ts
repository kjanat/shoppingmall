...
stubDocument();
const { Box3, Frustum, Matrix4, PerspectiveCamera, Scene, Sphere, Vector3 } = await import('three');
...
describe('on the pavement with the facade behind you', () => {
	...
	test('a sphere in the middle of V0 is rejected', () => {
		look(from, to, 'stad');
		expect(culler.accepts(zoneBit('mall-v0'), new Sphere(new Vector3(0, V0 + 2, 0), 2)), 'it is still accepted').toBeFalse();
	});
});
...
describe('on the pavement facing the doors', () => {
	...
	test('what stands just inside the doors is kept', () => {
		look(from, to, 'stad');
		const insideTheDoor = new Sphere(new Vector3(ENTRANCE_PORTAL.innerX + 3, V0 + EYE, PAVEMENT_Z), 1);
...
describe('deep inside V0 with the entrance behind you', () => {
	...
	test('something on the pavement behind you is rejected', () => {
		look(from, to, 'mall-v0');
		const onTheStreet = new Sphere(new Vector3(ENTRANCE_PORTAL.outerX - 6, V0 + EYE, ENTRANCE_PORTAL.centerZ), 1);
...
describe('on V1 looking straight up', () => {
	...
	test('the roof slab is kept anyway', () => {
		...
		const sphere = new Sphere(new Vector3(from.x, midpoint(bounds.minY, bounds.maxY), from.z), 2);
...
describe('on V1 looking down through the atrium', () => {
	...
	test('the ground floor slab is kept', () => {
		...
		if (!volume) return;
		const bounds = geometryBounds(volume.geometry);
		const sphere = new Sphere(new Vector3(0, midpoint(bounds.minY, bounds.maxY), 0), 2);
...
describe.each(VERTICAL_CONNECTORS.filter((connector) => connector.kind === 'escalator').map((connector) => connector.id))(
	...
	(id) => {
		...
		const rideCamera = new PerspectiveCamera(70, 16 / 9, 0.1, 500);
		...
		const box = new Box3().setFromObject(sign, true);
		...
		const ride = Array.from({ length: RIDE_STEPS + 1 }, (_value, index) => {
			...
			return {
				y,
				z,
				inView: frustum.intersectsSphere(new Sphere(center.clone(), radius)),
