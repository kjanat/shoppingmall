...
export function motorcycleMesh(color: number, track: <T extends Material>(material: T) => T): Group {
	...
	const lamp = track(new MeshBasicMaterial({ color: 0xfff3c4, toneMapped: false }));

	const tank = new Mesh(new BoxGeometry(body.width, body.height, body.length), paint);
	tank.position.y = body.centerY;
	group.add(tank);

	const saddle = new Mesh(new BoxGeometry(seat.width, seat.height, seat.length), rubber);
	saddle.position.set(0, seat.centerY, seat.offsetZ);
	group.add(saddle);

	for (const side of [-1, 1] as const) {
		const tyre = new Mesh(new CylinderGeometry(wheel.radius, wheel.radius, wheel.width, 12), rubber);
	...
	const handlebar = new Mesh(new BoxGeometry(bars.width, bars.thickness, bars.thickness), chrome);
	handlebar.position.set(0, bars.centerY, bars.offsetZ);
	group.add(handlebar);

	const koplamp = new Mesh(new SphereGeometry(headlight.radius, 10, 8), lamp);
