...
export class CityTheatre {
	...
	private buildBlok(): void {
		const steen = new THREE.MeshLambertMaterial({ color: 0xcfc5ad });
		const donker = new THREE.MeshLambertMaterial({ color: 0x3c3833 });
		const deur = new THREE.MeshLambertMaterial({ color: 0x2a211b });
	...
	private buildZuilen(): void {
		const steen = new THREE.MeshLambertMaterial({ color: 0xd8cfba });
	...
	private buildMarquee(): THREE.InstancedMesh {
		const bordeaux = new THREE.MeshLambertMaterial({ color: 0x531523 });
		const gloed = new THREE.MeshBasicMaterial({ color: 0xffdf9e, toneMapped: false });
		const staal = new THREE.MeshLambertMaterial({ color: 0x6b7078 });
	...
	private buildBorden(): THREE.MeshBasicMaterial {
		const bordeaux = new THREE.MeshLambertMaterial({ color: 0x531523 });
		const goudLijst = new THREE.MeshLambertMaterial({ color: 0x8a6d2f });
	...
	private buildTapijt(): void {
		const rood = new THREE.MeshLambertMaterial({ color: 0x9c0f2e });
		const goud = new THREE.MeshLambertMaterial({ color: 0xc9a227 });
		const koord = new THREE.MeshLambertMaterial({ color: 0x7a1230 });
