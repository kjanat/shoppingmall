 ...
 */
export const CON_RING_OUTER_X = 55;

/** One body height in world metres. Every con figure is a multiple of this. */
export const CON_BODY = STANDING_PEDESTRIAN.bodyHeight;
...
export const CON_GROUND_Y = 0;
export const CON_FLOOR_Y = 0.05;
export const CON_BASE_Y = -0.5;
export const CON_WALL_T = 0.5;

/** Exhibition hall free height: ~9 body heights (big-hall / arena feel). */
export const CON_HALL_HEIGHT = CON_BODY * 9;
export const CON_ROOF_T = 0.9;
export const CON_CEILING_Y = CON_HALL_HEIGHT - CON_ROOF_T;

/** Hotel storey: standing headroom + slab. */
export const CON_HOTEL_FLOOR_H = STANDING_PEDESTRIAN.requiredHeadroom + 1.0;
export const CON_HOTEL_FLOORS = 6;

/** Outdoor plaza on the city-facing (west) side — stays in `stad`. */
export const CON_PLAZA: Bounds2 = {
 ...
 */
export const CON_FOOTPRINT: Bounds2 = {
	minX: SHELL_WEST_X,
...
export const CON_DEALERS: Bounds2 = shellFrac(0, 0.58, 0, 0.54);

/** Main techno hall — west-centre, north. */
export const CON_STAGE: Bounds2 = shellFrac(0, 0.47, 0.58, 1);

/** Hotel — east, north. */
export const CON_HOTEL: Bounds2 = shellFrac(0.62, 1, 0.54, 1);

/** Adult wing — east of dealers, south (small rooms, human scale). */
export const CON_DARKROOM: Bounds2 = shellFrac(0.59, 0.72, 0.04, 0.32);
export const CON_STUDIO: Bounds2 = shellFrac(0.73, 0.91, 0.04, 0.32);

export const CON_ADULT_WING: Bounds2 = {
...
export const CON_ENVELOPE: Bounds2 = {
...
export const CON_ZONE_VOLUME: Bounds3 = {
	...CON_ENVELOPE,
	minY: CON_BASE_Y,
	maxY: CON_HALL_HEIGHT + CON_HOTEL_FLOORS * CON_HOTEL_FLOOR_H,
};

export const CON_CENTER = {
...
export const CON_ADULT_GATE_LEAF: Bounds3 = {
...
export const CON_ADULT_GATE_TRIGGER: Bounds3 = {
 ...
 */
export const CON_CROWD_COUNT = Math.round(clamp(WALK_AREA / 55, 80, 280));

export const CON_TECHNO_SOURCE = {
...
export const CON_CITY_MARGIN = 22;

export function rectInterior(rect: Bounds2, wall = CON_WALL_T): Bounds2 {
	return {
		minX: rect.minX + wall,
		maxX: rect.maxX - wall,
		minZ: rect.minZ + wall,
...
export function rectCenter(rect: Bounds2): { x: number; z: number } {
	return { x: midpoint(rect.minX, rect.maxX), z: midpoint(rect.minZ, rect.maxZ) };
}

export function rectSize(rect: Bounds2): { w: number; d: number } {
	return { w: span(rect.minX, rect.maxX), d: span(rect.minZ, rect.maxZ) };
}

export function inRect2(rect: Bounds2, x: number, z: number): boolean {
	return x >= rect.minX && x <= rect.maxX && z >= rect.minZ && z <= rect.maxZ;
}

export function inBounds3(box: Bounds3, x: number, y: number, z: number): boolean {
...
export function conBoothGrid(): { cols: number; rows: number; startX: number; startZ: number; pitchX: number; pitchZ: number } {
