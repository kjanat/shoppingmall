...
type SourceInstance = { mesh: THREE.Mesh<THREE.BufferGeometry, ColorMaterial>; instanceId: number };
	...
	update(): void {
		...
		for (const batch of this.batches) {
			for (const source of batch.sources) {
				batch.mesh.setMatrixAt(source.instanceId, source.mesh.matrixWorld);
				batch.mesh.setColorAt(source.instanceId, instanceColor(source.mesh.material));
				batch.mesh.setVisibleAt(source.instanceId, isVisible(source.mesh));
			}
		}
	}
}
